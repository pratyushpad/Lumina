...
When a Tracer is supplied, each stage records its candidates, scores, and latency.
"""
import logging
import time
...
from app.services.tracing import Tracer, candidates_payload
        ...
        if cfg.query_transform == "multi_query":
            ...
            if tracer:
                tracer.stage(
                    ...
                    {"kind": "multi_query", "queries": queries},
                )
        ...
        if (
            ...
            if tracer:
                tracer.stage(
                    "hybrid_sql",
                    int((time.perf_counter() - t0) * 1000),
                    candidates_payload([(r.chunk_id, r.relevance_score) for r in candidates]),
                )
        else:
            ranked_lists: list[list[str]] = []
            for q in queries:
                ranked_lists.extend(await self._ranked_lists_for(q, document_ids, tracer))
            t0 = time.perf_counter()
            ...
            if tracer:
                tracer.stage(
                    "fusion",
                    int((time.perf_counter() - t0) * 1000),
                    {"rrf_k": cfg.rrf_k, "lists": len(ranked_lists),
                     **candidates_payload(fused)},
                )

        if cfg.rerank:
            # Rerank always uses the ORIGINAL query — transforms only widen recall.
            t0 = time.perf_counter()
            reranked = Reranker.get().rerank(query, candidates, top_k=cfg.top_k_final)
            if tracer:
                tracer.stage(
                    "rerank",
                    int((time.perf_counter() - t0) * 1000),
                    candidates_payload(
                        [(r.chunk_id, r.relevance_score) for r in reranked]
                    ),
                )
            return reranked
    ...
    async def _ranked_lists_for(
        ...
        if cfg.mode in ("dense", "hybrid_rrf"):
            ...
            if cfg.query_transform == "hyde":
                ...
                if tracer:
                    tracer.stage("query_transform", 0, {"kind": "hyde", "passage": embed_text[:500]})
            ...
            if tracer:
                tracer.stage(
                    "dense",
                    int((time.perf_counter() - t0) * 1000),
                    {"query": q, **candidates_payload([(r.chunk_id, r.distance) for r in dense])},
                )
        ...
        if cfg.mode in ("sparse", "hybrid_rrf"):
            ...
            if tracer:
                tracer.stage(
                    f"sparse_{cfg.sparse_method}",
                    int((time.perf_counter() - t0) * 1000),
                    {"query": q, **candidates_payload(ranked)},
                )
